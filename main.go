...
func main() {
	bot, err := tgbotapi.NewBotAPI("2122811819:AAEIH8ooKHvh76MmeuEmlQLcc0xg3GFuq2M")
	if err != nil {
		log.Panic(err)
	}
	...
	for update := range updates {
		...
		case "ADD":
			...
			msg := fmt.Sprintf("Балланс: %s %f", msgArr[1], db[update.Message.Chat.ID][msgArr[1]])
		...
		case "SHOW":
			...
			for key, value := range db[update.Message.Chat.ID] {
				...
				rubSumm += value * coinPrice
				msg += fmt.Sprintf("%s: %f[%.2f руб]\n", key, value, value*coinPrice)
			}

			msg += fmt.Sprintf("Сумма: %.2f руб", rubSumm)

			bot.Send(tgbotapi.NewMessage(update.Message.Chat.ID, msg))

		default:
			bot.Send(tgbotapi.NewMessage(update.Message.Chat.ID, "Неизвестная команда, доступные команды: \n ADD <название валюты> Сумма \n SUB <название валюты> Сумма \n DEL<название валюты> \n SHOW"))
		}
...
func getPrice(coin string) (price float64, err error) {
	...
	resp, err := http.Get(fmt.Sprintf("https://api.binance.com/api/v3/ticker/price?symbol=%sRUB", coin))
	if err != nil {
		return
	}
